...
int main() {
  ...
  while (true) {
    double leftY = mast.Axis3.position() * 0.12;
    double leftX = mast.Axis4.position() * 0.12;
    ...
    if(absX >= absY){
      // y x 
      drive::arcade(ny / absX * r, sgn(nx) * r);
    }else {
      drive::arcade(sgn(ny) * r, nx / absY * r);
    }
    wait(10, msec);
  }
}
